  ...
  return (
    <div className="flex min-h-screen flex-col" style={{ background: "var(--color-bg-app)" }}>
      ...
      <ErrorBoundary fallbackTitle="Settings panel failed">
        <SettingsPanel isOpen={isSettingsOpen} settingsDraft={settingsDraft} cacheInfo={{ lastUpdated: cacheLastUpdated, accountCount: cacheAccountCount }} cachePrefs={cachePrefs} notificationPermission={notificationPermission} settingsSaving={settingsSaving} sendingTestNotification={sendingTestNotification} refreshIntervalError={refreshIntervalError} appVersion={appVersion} updateStatus={updateStatus} latestVersion={latestVersion} updateMessage={updateMessage} onClose={() => setIsSettingsOpen(false)} onFieldChange={handleSettingsField} onSave={() => void handleSaveSettings()} onClearCache={() => { clearCache(); showToast("Cache cleared"); }} onCachePrefsChange={setCachePrefs} onRequestNotificationPermission={() => void handleRequestNotificationPermission()} onSendTestNotification={() => void handleSendTestNotification()} onOpenGitHub={() => void handleOpenGitHub()} onCheckForUpdates={() => void handleCheckForUpdates()} />
      </ErrorBoundary>
