...
interface AccountGridProps {
  ...
  loadedBestByProvider: Array<{ provider: Provider; account?: AccountWithUsage }>;
  ...
  onSwitchBest: (provider: Provider) => void;
    ...
    <main className="max-w-6xl mx-auto px-6 py-8">
      {props.loading && props.accounts.length === 0 ? <div className="flex flex-col items-center justify-center py-20"><div className="animate-spin h-10 w-10 border-2 border-t-transparent rounded-full mb-4" style={{ borderColor: "var(--color-text-primary)", borderTopColor: "transparent" }} /><p style={{ color: "var(--color-text-secondary)" }}>Loading accounts...</p></div> : props.error ? <div className="text-center py-20"><div className="mb-2 text-red-600">Failed to load accounts</div><p className="text-sm" style={{ color: "var(--color-text-secondary)" }}>{props.error}</p></div> : props.filteredAccounts.length === 0 && props.activeAccounts.length === 0 ? <div className="text-center py-20"><div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl" style={{ background: "var(--color-bg-muted)" }}><span className="text-3xl">👤</span></div><h2 className="mb-2 text-xl font-semibold" style={{ color: "var(--color-text-primary)" }}>No accounts for this filter</h2><p className="mb-6" style={{ color: "var(--color-text-secondary)" }}>Adjust provider/tag/search filters or add a new account</p><button onClick={props.onAddAccount} className="rounded-lg px-6 py-3 text-sm font-medium sf-btn-primary">Add Account</button></div> : <div className="space-y-6">
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          {props.loadedBestByProvider.map(({ provider, account }) => <div key={provider} className="rounded-2xl p-4 sf-panel"><div className="flex items-center justify-between gap-3"><div><div className="mb-1"><Badge variant="provider" provider={provider} label={provider} /></div><div className="text-base font-semibold" style={{ color: "var(--color-text-primary)" }}>{account ? account.name : "No switchable account"}</div></div><button onClick={() => props.onSwitchBest(provider)} disabled={provider === "gemini" || !account} className="rounded-lg px-3 py-2 text-sm font-medium disabled:opacity-50" style={{ border: "1px solid color-mix(in srgb, var(--color-codex) 25%, transparent)", background: "color-mix(in srgb, var(--color-codex) 12%, transparent)", color: "var(--color-codex)" }}>{provider === "gemini" ? "Usage Only" : "Switch Best"}</button></div><div className="mt-3 text-sm" style={{ color: "var(--color-text-secondary)" }}>{account ? <><div>{formatPlanLabel(account.usage?.plan_type ?? account.plan_type, account.auth_mode)} • {getRemainingPercent(account).toFixed(1)}% remaining</div><div>{formatResetAt(account.usage?.primary_resets_at)}</div></> : <div>No current recommendation</div>}</div></div>)}
        </section>
