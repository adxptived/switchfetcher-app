...
use crate::auth::{
    add_account, load_accounts, push_account_action, set_active_account, switch_to_account,
    touch_account,
};
use crate::tray;
use crate::types::{AccountActionKind, AccountInfo, OAuthLoginInfo};
...
#[tauri::command]
pub async fn complete_login(app: tauri::AppHandle) -> Result<AccountInfo, String> {
    let pending = {
        let mut pending = PENDING_OAUTH.lock().unwrap();
        pending
            .take()
    ...
    push_account_action(
        ...
        format!("Completed OAuth login for {}", stored.name),
        None,
        false,
    )
    .map_err(|e| e.to_string())?;
    ...
    let (last_action, last_refresh_error) =
        crate::account_features::summarize_account_history(&stored, &store.history);
